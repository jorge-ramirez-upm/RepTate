...
.. autoclass:: RepTate.theories.TheoryStickyReptation.TheoryStickyReptation()
...
The relaxation modulus, :math:`G(t)`, is modeled as the sum of a Sticky-Rouse, :math:`G_\mathrm{SR}(t)` :cite:`LVE-Leibler1991` and a Double-Reptation, :math:`G_\mathrm{rep}(t)` :cite:`LVE-desCloizeaux1990` contribution,
...
(Note that this is approximate: after sticker dissociation a chain with a length twice :math:`N/Z_\mathrm{s}` or more relaxes,  see :cite:`LVE-Leibler1991` and :cite:`LVE-Rubinstein2001` )
...
and with :math:`g(x)=\sum_{m=1}^{\infty}m^{-2}\left[1-\exp(-m^2 x)\right]`. The dimensionless constant :math:`\alpha`: is in principle universal, but in practice varies between different polymers (see e.g. Ref. :cite:`LVE-Ruymbeke2002`). Note that :math:`Z_\mathrm{e}/\alpha` is equivalent to the parameter :math:`H` in Ref. :cite:`LVE-desCloizeaux1990`.
