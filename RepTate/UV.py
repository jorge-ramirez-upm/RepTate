...
def start_RepTate(argv):
    ...
    if inifile==None:
        tmpex.logger.error('INI file describing the data not found in the first file folder')
        sys.exit()

    ex = ApplicationUniversalViewer("UniversalViewer", tmpex, inifile=inifilepath+os.sep+inifile, nplot_max=nplot_max)
    ex.setWindowIcon(QIcon('RepTate/gui/Images/UView_Icon.ico'))
    ex.setWindowTitle('RepTate %s %s - Universal Viewer - INI: %s'%(tmpex.version, tmpex.date, inifile))
