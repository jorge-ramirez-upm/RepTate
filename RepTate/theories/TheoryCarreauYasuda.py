...
class TheoryCarreauYasuda(CmdBase):
    ...
    def __new__(cls, name='ThCarreauYasuda', parent_dataset=None, ax=None):
        ...
        Returns:
            [type] -- [description]
        """
        return GUITheoryCarreauYasuda(name, parent_dataset, ax) if (CmdBase.mode==CmdMode.GUI) else CLTheoryCarreauYasuda(name, parent_dataset, ax)
...
class BaseTheoryCarreauYasuda:
    ...
    def __init__(self, name='ThCarreauYasuda', parent_dataset=None, ax=None):
        ...
        self.parameters['etainf'] = Parameter(name='etainf', value=0, description='infinite shear rate viscosity', 
                                              type=ParameterType.real, opt_type=OptType.opt)
        self.parameters['lambda'] = Parameter(name='lambda', value=100, description='characteristic time',
                                              type=ParameterType.real, opt_type=OptType.opt)
        self.parameters['n'] = Parameter(name='n', value=0.2, description='characteristic exponent',
                                         type=ParameterType.real, opt_type=OptType.opt)
        self.parameters['a'] = Parameter(name='a', value=1.0, description='exponent a',
                                         type=ParameterType.real, opt_type=OptType.opt)
