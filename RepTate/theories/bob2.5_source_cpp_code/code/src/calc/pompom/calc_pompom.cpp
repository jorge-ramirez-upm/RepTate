...
for(int i=0; i<kmax; i++){xp[i]=exp(log(tmin) + ((double) i)*dlx); 
                          stress[i]=N1[i]=0.0;}

double gm, g0, tauB, tauS, stretchrate, axx, ayy, axy;
int q, num_maxwell, num_modes;

FILE * maxfl=fopen("maxwell.dat","r"); FILE * nlinfl=fopen("nlin_modes.dat","r");
...
fscanf(nlinfl, "%lf %d",&tauB, &num_modes);
     for(int jm=0; jm<num_modes; jm++){
       ...
       g0=g0*gm; tauS=tauB/stretchrate;

double x1=0.0; double x2=tmax; double ystart=1.0; double eps=1.0e-4; 
double h1=0.01/gdot; if(h1 > (tmin/2.0)){h1=tmin/2.0;}
 if(h1 < (tauS/100.0)){h1=tauS/100.0;}
...
for(int k=0; k<kmax; k++){
/* yp[k]=1.0; check */
if( ((2.0*gdot*tauB - 1.0)*xp[k]/tauB) > 60.0){axx=1.0; ayy=0.0;}
else{axx=(1.0 - 2.0*gdot*tauB*exp((2*gdot*tauB - 1.0)*xp[k]/tauB))/(1.0 - 2*gdot*tauB);
ayy=(1.0+gdot*tauB*exp(-(1.0+gdot*tauB)*xp[k]/tauB)) /(1.0+gdot*tauB); }
if(axx > 1.0e30){stress[k]+=3.0*g0*yp[k]*yp[k];}
else{stress[k]+=3.0*g0*yp[k]*yp[k]*(axx - ayy)/ (axx + 2.0*ayy); }
    } }
}}

fclose(maxfl); fclose(nlinfl);
}
