...
double mwav, mnav, pdi; mwav=mnav=0.0;
double cur_wt_fraction=0.0;
for(int i=ni; i<nf; i++){cur_wt_fraction+=wtfrac_ar[i-ni]; }

 for(int i=ni; i<nf; i++){mnav+=wtfrac_ar[i-ni]/mass_ar[i-ni];
...
delete [] mass_ar; delete [] gfac_ar; delete [] branch_ar; delete [] wtfrac_ar;
}
