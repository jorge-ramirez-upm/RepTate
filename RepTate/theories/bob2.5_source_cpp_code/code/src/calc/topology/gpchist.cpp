...
{
extern int GPCNumBin; extern double mass_mono;
int nnn=GPCNumBin;
if(nnn > (n_cur_comp/5)){nnn=n_cur_comp/5;}
if(nnn < 2){nnn=2;}
double * wtbin=new double [nnn];
double * brbin=new double [nnn];
double * gbin=new double [nnn];
for(int i=0; i<nnn; i++){wtbin[i]=brbin[i]=gbin[i]=0.0;}

double massmin=1.0e20; double massmax=0.0;
for(int i=0; i<n_cur_comp; i++){
if(massmin > mass_ar[i]){massmin=mass_ar[i];}
if(massmax < mass_ar[i]){massmax=mass_ar[i];} }

if( (massmax - massmin) < 0.01){
   printf("Polymers are too monodisperse for GPC histogram \n");
                               }
else{
double lgmin=log10(massmin); double lgmax=log10(massmax);
double lgstep=(lgmax - lgmin)/((double) nnn);

int indx; double wttot=0.0;
for (int i=0; i<n_cur_comp; i++){
indx= (int) floor((log10(mass_ar[i]) - lgmin)/lgstep);
indx++;
if(indx > (nnn-1)){indx=nnn-1;}
...
for(int i=1;i<nnn; i++){
if(wtbin[i] > 1.0e-12){ gbin[i]=gbin[i]/wtbin[i];}
if(wtbin[i] > 1.0e-12){ brbin[i]=brbin[i]/wtbin[i];}
wtbin[i]=wtbin[i]/(lgstep*wttot);}

dumpgpcres(ncomp, nnn, wtbin, brbin, gbin, lgmin, lgstep);
   }
delete [] wtbin; delete [] brbin; delete [] gbin;
} 
