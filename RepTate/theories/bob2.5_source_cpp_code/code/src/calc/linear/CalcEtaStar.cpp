...
fclose(fid);
double * ff, *competa;
ff=new double[ndata]; competa=new double [ndata];

fid=fopen("gtp.dat","r");
for(int i=0; i<ndata; i++){
  err=fscanf(fid,"%lf %lf %lf", &a1,&a2,&a3);
  ff[i]=a1; competa[i]=sqrt(a2*a2+a3*a3)/a1;
                          }
fclose(fid);
extern double interp_logscale(int, double, double *, double *);
eta=interp_logscale(ndata, freq, ff, competa);
delete [] ff; delete [] competa;
    }
return eta;
}
...
double val=0.0;
int k1=0, k2=0; 
if(x[0] < x[nd -1]){ // x increasing
 for(int i=0; i<nd; i++){if(k1==0){if(xx < x[i]){k1=1; k2=i;}}}
 if(k1 == 0){k2=nd-1;}
 if(k2 == 0){k2=1;}
...
double lx1=log(x[k2-1]); double lx2=log(x[k2]);
double ly1=log(y[k2-1]); double ly2=log(y[k2]);
