...
extern void rcdefault(void); rcdefault();

FILE * flrc=fopen("bob.rc","r");
extern FILE * infofl;
if(flrc != NULL){
fprintf(infofl,"Found bob.rc \n");
  while(err != -1){ err=getline(flrc, linedata);
  if(err != -1){err=splitrcopt(linedata, rccom, rcval);
                if(err!=-1){
         removewhitespace(rccom); removewhitespace(rcval);
