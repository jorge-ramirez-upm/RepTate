...
if(runmode == 2) {
printf("\nPlease enter dilation exponent alpha : "); scanf("%le", &Alpha);
printf("\n");
...
int tint; scanf("%d", &tint);
printf("\n");
if(tint ==1) stored_values = true;
if(! stored_values){
...
printf("       Please enter R_L : "); scanf("%le", &RetLim); printf("\n");
printf("Beyond retraction time, branch point diffuses with hop size \"p.a\" \n");
printf("Value of p^2 ?  "); scanf("%le", &PSquare); printf("\n");
...
extern FILE * inpfl; fscanf(inpfl, "%le", &Alpha);
int tmpint; fscanf(inpfl, "%d", &tmpint); 
if(tmpint != 1){
...
else {stored_values = true;} }

if(RetLim < 1e-8) {RetLim = 1e-8;}
