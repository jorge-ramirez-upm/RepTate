    ...
    * **Parameters**
    ...
    def __new__(cls, name="", parent_dataset=None, ax=None):
        ...
        return GUITheoryWLFShiftTest(
            name, parent_dataset,
            ax) if (CmdBase.mode == CmdMode.GUI) else CLTheoryWLFShiftTest(
...
class BaseTheoryWLFShiftTest:
    ...
    [description]
    """
    help_file = 'http://reptate.readthedocs.io/en/latest/manual/Applications/TTS/Theory/theory.html#williams-landel-ferry-tts-shift-test'
    single_file = False
    ...
    def do_error(self, line):
        ...
        if (line == ""):
            self.Qprint("%5s %5s %5s %5s %10s (%10s)" %
                        ("Mw", "Mw2", "phi", "phi2", "Error", "# Points"))
            self.Qprint("=====================")
            p = list(MwUnique.keys())
            p.sort()
            for o in p:
                if (MwUnique[o][1] > 0):
                    self.Qprint("%5gk %5gk %5g %5g %10.5g (%10d)" %
                                (o[0], o[1], o[2], o[3],
                                 MwUnique[o][0] / MwUnique[o][1],
                                 MwUnique[o][1]))
                else:
                    self.Qprint("%5gk %5gk %5g %5g %10s (%10d)" %
                                (o[0], o[1], o[2], o[3], "-", 0))
        if (npoints > 0):
            total_error /= npoints
        else:
            total_error = 1e10
        if (line == ""):
            self.Qprint("%21s %10.5g (%10d)" % ("TOTAL", total_error, npoints))
        if (line == ""):
            self.Qprint("")
            B1 = self.parameters["B1"].value
            B2 = self.parameters["B2"].value
            Tr = self.parameters["Tr"].value
            self.Qprint("WLF Params @ Tr = %g" % Tr)
            self.Qprint("C1 = %g" % (B1 / (B2 + Tr)))
            self.Qprint("C2 = %g" % (B2 + Tr))
