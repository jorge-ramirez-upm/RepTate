...
/* local functions */
static void diene_grow(int dir, int m, double cur_conv, bool sc_tag);
...
void diene_grow(int dir, int m, double cur_conv, bool sc_tag)
{
    ...
    if ((rlevel > 5000) || dCSTR_global.dieneCSTRerrorflag)
    {
        ...
        { // a branch point
            ...
            if (request_arm(&m1))
            {
                ...
                if (request_arm(&m2))
                {
                    ...
                    arm_pool[m].arm_len = seg_len;
                    arm_pool[m1].arm_conv = cur_conv;
                    diene_grow(dir, m1, cur_conv, false);
                    ...
                    if (request_arm(&m3))
                    {
                        ...
                        if (request_arm(&m4))
                        {
                            ...
                            diene_grow(-1, m3, new_conv, true);
                            diene_grow(1, m4, new_conv, true);
                        } // end arm available
                    }     // end arm available
                }         // end arm available
            }             // end arm available
