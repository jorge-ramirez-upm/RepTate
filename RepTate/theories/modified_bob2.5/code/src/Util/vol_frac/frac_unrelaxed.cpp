...
if(!arm_pool[n1].relaxing){ phi_mol+= arm_pool[n1].vol_fraction;}
  n2=arm_pool[n1].down;
   while(n2 != n1)
    {
if(!arm_pool[n2].relaxing){ phi_mol+= arm_pool[n2].vol_fraction;}
      n2=arm_pool[n2].down;
    }
phi_true+=phi_mol;
if(LateRouse == 0){LtRsLHS+=branched_poly[i].wtfrac*
...
if(LateRouse == 0){
extern double LtRsFactor; extern int LtRsActivated;
if(LtRsActivated != 0){
if(LtRsLHS <  LtRsFactor){LtRsActivated = 0; 
 ...
 if(phi_true < (phi* exp(-log(DtMult)/(2.0*Alpha))))
  {
   ...
   phi_ST_0=phi; ST_activ_time=cur_time/DtMult;
   phi_ST=phi * exp(-log(DtMult)/(2.0*Alpha));
if(SlavePhiToPhiST == 0){return (phi_ST);}
else{ return(phi);}
  }
 else
  {
   phi_ST=phi_true; return(phi_true);
  }
}
else
{
...
if(SlavePhiToPhiST == 0){return (phi_ST);}
   ...
   {phi_ST=phi_true; supertube_activated=false; return(phi_true);}
