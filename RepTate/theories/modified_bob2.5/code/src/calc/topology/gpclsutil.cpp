...
int n0=branched_poly[i].first_end; double mass=arm_pool[n0].arm_len;
int nd=arm_pool[n0].down;
while(nd != n0){ mass+=arm_pool[nd].arm_len; nd=arm_pool[nd].down; }
extern double N_e, mass_mono; mass=mass*N_e * mass_mono;
return mass; }

double gpc_calc_wtfrac(int i) {
extern polymer * branched_poly; extern arm * arm_pool;
int n0=branched_poly[i].first_end; double mass=arm_pool[n0].vol_fraction;
int nd=arm_pool[n0].down;
while(nd != n0){ mass+=arm_pool[nd].vol_fraction; nd=arm_pool[nd].down; }
return mass; }

int gpc_num_br(int i) {
extern polymer * branched_poly; extern arm * arm_pool;
int n0=branched_poly[i].first_end; int nn=1;
int nd=arm_pool[n0].down;
while(nd != n0){ nn++; nd=arm_pool[nd].down;}
nn=(nn-1)/2; return nn; }
...
while(n2 != n1){nbr++; ntot+=arm_pool[n2].arm_len; n2=arm_pool[n2].down;}
...
if((arm_pool[n1].L1 == i) || (arm_pool[n1].L2 == i)){ // travel right
 int nR1=arm_pool[n1].R1; if(nR1 != -1){mass+=recur_left_mass(nR1, n1);}
 int nR2=arm_pool[n1].R2; if(nR2 != -1){mass+=recur_left_mass(nR2, n1);}
                                                    }
else{ // travel left
 int nL1=arm_pool[n1].L1; if(nL1 != -1){mass+=recur_left_mass(nL1, n1);}
 int nL2=arm_pool[n1].L2; if(nL2 != -1){mass+=recur_left_mass(nL2, n1);}
    }
return mass;
}
