...
double mass_poly(int n)
{
 ...
 while(n2 != n1) {mass_now += arm_pool[n2].vol_fraction; n2=arm_pool[n2].down;}
 return(mass_now);
}
...
int ncomb=0; int nlin=0; int nstar=0; int ncombp=0;
double mcomb=0.0; double mlin=0.0; double mstar=0.0; double mcombp=0.0;
double mtot=0.0; double mtmp; double n_branch=0.0;
extern int num_poly;
extern FILE * infofl;
fprintf(infofl,"Branching topology of the polymers\n");
 for (int i=0; i < num_poly; i++)
 {
  if(branched_poly[i].num_branch == 2)
   {nlin++;mtmp=mass_poly(i); mlin+=mtmp; mtot+=mtmp;}
  else {
      n_branch+=(double) ((branched_poly[i].num_branch -1)/2);
      if(branched_poly[i].num_branch == 3)
         {nstar++;mtmp=mass_poly(i);mstar+=mtmp;mtot+=mtmp;}
       else { if(iscomb(i))
                {ncomb++;mtmp=mass_poly(i);mcomb+=mtmp;mtot+=mtmp;}
               else{ncombp++;mtmp=mass_poly(i); mcombp+=mtmp;mtot+=mtmp; }
            }
     }
 }
