...
void flow_travel(int n0, int n1, int *m)
{
  ...
  int LL1 = arm_pool[n1].L1;
  int LL2 = arm_pool[n1].L2;
  int RR1 = arm_pool[n1].R1;
  int RR2 = arm_pool[n1].R2;
  ...
  { // travel right
    ...
    { // not a free end
      if ((RR1 != -1) && (RR2 != -1))
      {
        ...
        if ((arm_pool[RR1].armtaus < SnipTime) &&
            (arm_pool[RR2].armtaus >= SnipTime))
        { //need to decide on side arm contrib.
          if (arm_pool[RR1].str_time >= SnipTime)
          {
            m[0] += 1;
          }
          else
        ...
        if ((arm_pool[RR1].armtaus >= SnipTime) &&
            (arm_pool[RR2].armtaus < SnipTime))
        { //need to decide on side arm contrib.
          if (arm_pool[RR2].str_time >= SnipTime)
          {
            m[0] += 1;
          }
          else
      ...
      if ((RR1 == -1) && (RR2 != -1))
      { // only RR2 exists
        if (arm_pool[RR2].armtaus >= SnipTime)
        {
          flow_travel(n1, RR2, m);
        }
        else
    ...
    { // not a free end
      if ((LL1 != -1) && (LL2 != -1))
      {
        ...
        if ((arm_pool[LL1].armtaus < SnipTime) &&
            (arm_pool[LL2].armtaus >= SnipTime))
        { //need to decide on side arm contrib.
          if (arm_pool[LL1].str_time >= SnipTime)
          {
            m[0] += 1;
          }
          else
        ...
        if ((arm_pool[LL1].armtaus >= SnipTime) &&
            (arm_pool[LL2].armtaus < SnipTime))
        { //need to decide on side arm contrib.
          if (arm_pool[LL2].str_time >= SnipTime)
          {
            m[0] += 1;
          }
          else
      ...
      if ((LL1 == -1) && (LL2 != -1))
      { // only LL2 exists
        if (arm_pool[LL2].armtaus >= SnipTime)
        {
          flow_travel(n1, LL2, m);
        }
        else
  ...
  { // travel right
    ...
    { // both arms exist
      ...
      if ((arm_pool[RR1].str_time >= Time) &&
          (arm_pool[RR2].str_time < Time))
      {
        flow_lencal(n1, RR1, endlab, Time, zlength);
      } // one arm relaxed
      if ((arm_pool[RR1].str_time < Time) &&
          (arm_pool[RR2].str_time >= Time))
      {
        flow_lencal(n1, RR2, endlab, Time, zlength);
      } // one arm relaxed
    }
    if ((RR1 < 0) && (RR2 < 0))
    {
  ...
  { // travel left
    ...
    { // both arms exist
      ...
      if ((arm_pool[LL1].str_time >= Time) &&
          (arm_pool[LL2].str_time < Time))
      {
        flow_lencal(n1, LL1, endlab, Time, zlength);
      } // one arm relaxed
      if ((arm_pool[LL1].str_time < Time) &&
          (arm_pool[LL2].str_time >= Time))
      {
        flow_lencal(n1, LL2, endlab, Time, zlength);
      } // one arm relaxed
    }
    if ((LL1 < 0) && (LL2 < 0))
    {
