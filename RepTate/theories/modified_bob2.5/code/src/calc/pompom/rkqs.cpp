...
{
double SAFETY=0.9;double PGROW=-0.2;double PSHRNK=-0.25;double ERRCON=1.89e-4;
double h=htry; int cont=0;
...
if(errmax > 1.0){htemp=SAFETY*h*(exp(log(errmax)*PSHRNK));
tmpmax=fabs(htemp); if(tmpmax < (0.1*fabs(h))) { tmpmax=0.1*fabs(h);}
if(h > 0.0){h=fabs(tmpmax);}
else{h=-fabs(tmpmax);}
xnew=x[0]+h;
if(xnew == x[0]) {warnmsgs(403); h=1.0e-8;}
cont=0; 
}}

if(errmax > ERRCON){hnext[0]=SAFETY * h *(exp(log(errmax)*PGROW));}
else{hnext[0]=5.0*h;}

hdid[0]=h; x[0]+=h; y[0]=ytemp;  
