...
for(int i=0; i<max_prio_var; i++){
 for(int j=0; j<NumNlinStretch; j++){
  if(nlin_prio_phi_relax[i][j] > 1.0e-16){
   tot_phi_relax+=nlin_prio_phi_relax[i][j]; num_modes++; }
  if(nlin_prio_phi_held[i][j] > 1.0e-16){
   tot_phi_held+=nlin_prio_phi_held[i][j]; num_modes++; }} }
...
for(int i=0; i<max_prio_var; i++){ cur_rate=1.0;
 for(int j=0; j<NumNlinStretch; j++){
  if(nlin_prio_phi_relax[i][j] > 1.0e-16){
...
else{nlin_t_min=nlin_t_max=1.0e32;}
}
