...
void lin_time_resp(int ndata, double *tp, double *phip, double *phip_ST)
{
  ...
  double tt, goft_slow, goft_fast, phnow, phstnow;
  tt = 0.00010;
  ...
  for (int i1 = 0; i1 < 300; i1++)
  {
    ...
    for (int j = 1; j < ndata; j++)
    {
      ...
      goft_slow += exp(-tt / tp[j - 1]) *
                   (Alpha * phnow * pow(phstnow, Alpha) * (phip_ST[j - 1] - phip_ST[j]) / phstnow + pow(phstnow, Alpha) * (phip[j - 1] - phip[j]));
    }
    if (calc_gt_fast)
    {
      goft_fast = fast_real_hist(tt);
      if (goft_fast < (0.01 * goft_slow))
        calc_gt_fast = false;
    }
    else
    {
