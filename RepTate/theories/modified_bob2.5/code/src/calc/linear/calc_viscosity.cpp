...
double ff[5], eta[5];
int cont=0; int num_try=0;
double freq, phnow, phstnow,tnow, tmpvar, tmpvar1;
extern double Alpha, G_0_unit, unit_time;

ff[0]=1e-10; for(int i=1; i<5; i++) {ff[i]=ff[i-1]/2.0;}
while((cont == 0) && (num_try < 30)) { num_try++;
for(int i=0; i<5; i++){ freq=ff[i]; eta[i]=0.0;
for(int j=0; j<ndata; j++){
phnow=0.50*(phip[j-1]+phip[j]); phstnow=0.50*(phip_ST[j-1]+phip_ST[j]);
tnow=tp[j]; tmpvar=tnow/(1.0 + freq*freq*tnow*tnow);
...
tmpvar=0.0;
for(int i=1; i<5; i++){tmpvar+=(eta[i]-eta[i-1])*(eta[i]-eta[i-1]);}
if((tmpvar/eta[3]) < 1e-12){ cont=1;}
else { for(int i=0; i<5; i++){ff[i]=ff[i]/2.0;}}
}

double sx=0.0; double sy=0.0; double sxx=0.0; double sxy=0.0;
for(int i=0; i<5; i++){sx+=ff[i]; sy+=eta[i]; sxx+=ff[i]*ff[i];
         sxy+=eta[i]*ff[i];}
extern FILE * infofl;
if(num_try > 19){
