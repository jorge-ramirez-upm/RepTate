...
for (int zi=zintmin; zi <= zintmax; zi++) {
  ...
  if(phi_now >= 0.0) {
    ...
    gp=0.0; g2p=0.0;
      for (int i=1; i<zi; i++) {
          tmpvar=(double)i/zz;
          tmppow2=tmpvar*tmpvar; tmppow4=tmppow2*tmppow2;
          ...
          if(comp_term > 0.00010) comp_term=0.00010;
          int max_term = (int)ceil(N_e * zz);
           while(quit==0) {
             tmpvar=double(k)/zz;
             tmppow2=tmpvar*tmpvar; tmppow4=4.0*tmppow2*tmppow2;
             tmpvar=1.0/(freqsq + tmppow4);
             gp+=5.0*tmpvar;
             g2p+=10.0*tmppow2/(freqsq + tmppow4);
             if(tmpvar < comp_term) quit=1;
             if(k >= max_term) quit=1; k++; }
    gpfast[0]+= gp * phi_now/(4.0*zz); g2pfast[0]+= g2p * phi_now/(4.0*zz);
   }
 }
    gpfast[0]=freqsq * gpfast[0]; g2pfast[0]=freq * g2pfast[0];
