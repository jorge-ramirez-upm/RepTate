                 ...
                 double cs, double cb, double fin_conv, double tau, double beta, int *bcount)
{
    ...
    else
    {
        ...
        { // branch point
            ...
            arm_pool[m1].relaxing = arm_pool[m].relaxing;
        ...
        { // nonside branch
            ...
            else
            {
                if (dir > 0)
                {
                    sigma = cs * cur_conv / (1.0 - cur_conv);
                    lambda = cb * cur_conv / (1.0 - cur_conv);
                ...
                else
                {
                    ...
                    {
                        bcount[0]++;
                        new_conv = getconv1(cur_conv);
                        m1 = request_attached_arm(m);
                        arm_pool[m1].arm_len = calclength(new_conv, cs, cb, tau, beta);
                        arm_pool[m1].z = new_conv;
                        arm_pool[m1].relaxing = false;
                        arm_pool[m].L1 = m1;
                        arm_pool[m1].L2 = m;
                        tobita_grow(1, m1, new_conv, true, &rlevel[0], cs, cb, fin_conv, tau, beta, &bcount[0]);
                        m2 = request_attached_arm(m);
                        arm_pool[m2].arm_len = calclength(new_conv, cs, cb, tau, beta);
                        arm_pool[m2].z = new_conv;
                        arm_pool[m2].relaxing = false;
                        arm_pool[m].L2 = m2;
                        arm_pool[m2].R1 = m;
