...
if(nmin > nd){nmin=nd;}
if(nmax < nd){nmax=nd;}
while(nd != n0) { arm_pool[nd].tmpflag=true;
nd=arm_pool[nd].down; kk++;
if(nmin > nd){nmin=nd;}
if(nmax < nd){nmax=nd;}
}
...
kk=0; int found_it; int nk;


tobita_arm_clean(n0, &kk, tmp_pool);


while(kk != 0){ found_it=-1;
for(int i=0; i<kk; i++){
   if(tmp_pool[i] == nmax){found_it=0; return_arm(tmp_pool[i]);}
   if((found_it==0) && (i != (kk-1))){tmp_pool[i]=tmp_pool[i+1];}
                       }
if(found_it ==0){kk--; nmax--;}
else{ // the arm is in the polymer
   n0=cur_poly[0].first_end; nk=tmp_pool[kk-1];
      if(n0 == nmax){tobita_swap_arm(nk,n0); 
          kk--; nmax--; return_arm(n0); cur_poly[0].first_end=nk;}
      else{
        nd=arm_pool[n0].down;
      while(nd != nmax){nd=arm_pool[nd].down;}
         tobita_swap_arm(nk, nd); kk--; nmax--; return_arm(nd);
           }
    }
           }
...
n0=cur_poly[0].first_end; nd=arm_pool[n0].down;
if(n0 == nd){// we have a linear
...
arm_pool[n0].R1=nd; arm_pool[nd].L1=n0;
...
nt=arm_pool[n].L1; arm_pool[m].L1=nt;
if(nt != -1){
 if(arm_pool[nt].L1 == n){arm_pool[nt].L1=m;}
 if(arm_pool[nt].L2 == n){arm_pool[nt].L2=m;}
 if(arm_pool[nt].R1 == n){arm_pool[nt].R1=m;}
 if(arm_pool[nt].R2 == n){arm_pool[nt].R2=m;}
            }
nt=arm_pool[n].L2; arm_pool[m].L2=nt;
if(nt != -1){
 if(arm_pool[nt].L1 == n){arm_pool[nt].L1=m;}
 if(arm_pool[nt].L2 == n){arm_pool[nt].L2=m;}
 if(arm_pool[nt].R1 == n){arm_pool[nt].R1=m;}
...
nt=arm_pool[n].R1; arm_pool[m].R1=nt;
if(nt != -1){
 if(arm_pool[nt].L1 == n){arm_pool[nt].L1=m;}
 if(arm_pool[nt].L2 == n){arm_pool[nt].L2=m;}
 if(arm_pool[nt].R1 == n){arm_pool[nt].R1=m;}
...
nt=arm_pool[n].R2; arm_pool[m].R2=nt;
if(nt != -1){
 if(arm_pool[nt].L1 == n){arm_pool[nt].L1=m;}
 if(arm_pool[nt].L2 == n){arm_pool[nt].L2=m;}
 if(arm_pool[nt].R1 == n){arm_pool[nt].R1=m;}
