...
void genStar(int ni, int nf)
{
  ...
  else
  {
    ...
    printf(" How many arms the star has?  ");
    scanf("%d", &num_arm);
  }
  ...
  for (int i = ni; i < nf; i++)
  {
    ...
    {
    case 3:
      branched_poly[i] = polygenStar(arm_type, mass, pdi);
      break;
    case 4:
      branched_poly[i] = polygenH(arm_type, mass, pdi, 0, zeroarm, pdi);
      break;
    case 6:
      branched_poly[i] = polygenCayley(1, armtypev, massv, pdiv);
      break;
    case 18:
      branched_poly[i] = polygenStar18(arm_type, mass, pdi);
      break;
    }
  }
  if (!reptate_flag)
...
polymer polygenStar18(int arm_type, double mn_arm, double pdi)
{
  ...
  {
    nn[i] = request_arm();
    arm_pool[nn[i]].arm_len = 0.0;
  }
  for (int i = 15; i < 33; i++)
  {
    arm_pool[nn[i]].arm_len = poly_get_arm(arm_type, mn_arm, pdi);
  }
  for (int i = 1; i < 32; i++)
  {
    arm_pool[nn[i]].down = nn[i - 1];
    arm_pool[nn[i]].up = nn[i + 1];
  }
  arm_pool[nn[0]].down = nn[32];
  arm_pool[nn[0]].up = nn[1];
  arm_pool[nn[32]].down = nn[31];
  arm_pool[nn[32]].up = nn[0];
