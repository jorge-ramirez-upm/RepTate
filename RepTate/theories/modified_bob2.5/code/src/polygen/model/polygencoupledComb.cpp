                           ...
                           int arm_typea, double m_arm, double pdi_arm, double dnarm)
{
  ...
  double dnarm1 = poisson(dnarm) + 0.20;
  int n_arm = (int)floor(dnarm1);
  if ((dnarm1 - (double)n_arm) > 0.50)
  {
    n_arm++;
  }
  ...
  int n_arm1 = (int)floor(dnarm1);
  if ((dnarm1 - (double)n_arm1) > 0.50)
  {
    n_arm1++;
  }
  n_arm1++;
  ...
  if (storearm != -1)
  {
    ...
    n0p = request_arm();
    n1p = request_arm();
    arm_pool[storearm].L1 = n0p;
    arm_pool[storearm].L2 = n1p;
    arm_pool[n0p].R1 = storearm;
    arm_pool[n0p].R2 = n1p;
    arm_pool[n0p].L1 = arm_pool[n0p].L2 = -1;
    arm_pool[n1p].L1 = storearm;
    arm_pool[n1p].L2 = n0p;
    arm_pool[n1p].R1 = arm_pool[n1p].R2 = -1;

    if (couplepoint == 0)
    {
      arm_pool[n0p].arm_len = junc[0];
    }
    else
    {
      arm_pool[n0p].arm_len = junc[couplepoint] - junc[couplepoint - 1];
    }
    if (couplepoint != (n_arm1 - 1))
    {
      arm_pool[n1p].arm_len = junc[couplepoint + 1] - junc[couplepoint];
    }
    else
    {
      arm_pool[n1p].arm_len = cur_bbone - junc[n_arm1 - 1];
    }
    nd = arm_pool[storearm].down;
    arm_pool[storearm].down = n0p;
    arm_pool[n0p].up = storearm;
    arm_pool[n0p].down = n1p;
    arm_pool[n1p].up = n0p;
    arm_pool[n1p].down = nd;
    arm_pool[nd].up = n1p;
    ...
    if (couplepoint != 0)
    {
      ...
      for (int k = couplepoint - 1; k > 0; k--)
      {
        ...
        arm_pool[n1pa].R1 = n0pa;
        arm_pool[n0pa].L1 = n1pa;
        arm_pool[n1pa].arm_len = cur_arm;
        nd = arm_pool[n0].down;
        arm_pool[n0].down = n0pa;
        arm_pool[n0pa].up = n0;
        arm_pool[n0pa].down = n1pa;
        arm_pool[n1pa].up = n0pa;
        arm_pool[n1pa].down = nd;
        arm_pool[nd].up = n1pa;
        n0 = n0pa;
        n1 = n1pa;
      }
      n0pa = request_arm();
      arm_pool[n0pa].arm_len = junc[0];
