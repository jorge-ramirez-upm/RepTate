...
if(runmode == 2){ printf("Information about the backbone .. \n");
  user_get_arm_type(&arm_typeb,&m_bbone,&pdi_bbone); }
else{fscanf(inpfl, "%d %lf %lf",&arm_typeb, &m_bbone, &pdi_bbone);}
if(runmode == 2){printf("Information about the side arms .. \n");
  user_get_arm_type(&arm_typea,&m_arm,&pdi_arm);}
else{fscanf(inpfl, "%d %lf %lf",&arm_typea, &m_arm, &pdi_arm);}
if(runmode == 2){ printf("Average number of side arms per molecule ?  ");
  scanf("%le", &num_arm); }
else{fscanf(inpfl, "%lf", &num_arm);}
