...
void poly_start(polymer *cur_poly)
{
  ...
  cur_poly[0].alive = true;
  cur_poly[0].relaxed_frac = 0.0;
  cur_poly[0].rept_set = false;
  cur_poly[0].ghost_contrib = 0.0;
  int n1 = cur_poly[0].first_end;
  arm_start(n1);
  int num_seg = 1;
  ...
  if (num_seg <= 3) // star or linear
  {
    ...
    arm_pool[n1].freeze_arm_len_eff = true;
    n2 = arm_pool[n1].down;
    while (n2 != n1)
    {
      arm_pool[n2].freeze_arm_len_eff = true;
      n2 = arm_pool[n2].down;
    }
  }
  ...
  {
    cur_poly[0].first_free = n1;
    found_free = true;
    arm_pool[n1].free_up = n1;
    arm_pool[n1].free_down = n1;
  }
  n2 = arm_pool[n1].down;
  while (n2 != n1)
  {
    if (arm_pool[n2].free_end)
    {
      if (!found_free)
      {
        cur_poly[0].first_free = n2;
        found_free = true;
        arm_pool[n2].free_up = n2;
        arm_pool[n2].free_down = n2;
      }
      else
      {
        int n3 = cur_poly[0].first_free;
        int n4 = arm_pool[n3].free_up;
        arm_pool[n3].free_up = n2;
        arm_pool[n2].free_down = n3;
        arm_pool[n2].free_up = n4;
        arm_pool[n4].free_down = n2;
      }
    }
    n2 = arm_pool[n2].down;
  }
