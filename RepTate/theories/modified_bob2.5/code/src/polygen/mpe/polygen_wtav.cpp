...
{ // dir >0 => downstream
  ...
  if (dir > 0)
  {
    ...
    {
      add_wt_right(n1, logprob, b_u, b_d, 1);
    }
  }
  else
  {
  ...
  {
    add_wt_right(n2, logprob, b_u, b_d, -1);
  }
}
...
polymer polygen_wtav(int ptype, double logprob, double b_u)
{
  ...
  if (mtrand1() < b_u)
  {
    add_wt_left(n1, logprob, b_u);
  }
  if (mtrand1() < b_d)
  {
    add_wt_right(n1, logprob, b_u, b_d, 1);
  }
  if ((arm_pool[n1].L1 == -1) && (arm_pool[n1].R1 == -1))
  { //linear
    int n2 = request_arm();
    double seglen = arm_pool[n1].arm_len;
    arm_pool[n1].up = n2;
    arm_pool[n1].down = n2;
    arm_pool[n2].up = n1;
    arm_pool[n2].down = n1;
