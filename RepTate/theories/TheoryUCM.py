...
class TheoryUCMLAOS(CmdBase):
    thname = "UCMLAOS"
    description = "Upper-convected Maxwell constitutive equation LAOS"
    citations = "Oldroyd J.G., Proc. Roy. Soc. 1950, 200, 523-541"
    doi = "http://dx.doi.org/10.1098/rspa.1950.0035"
...
class BaseTheoryUCMLAOS:
    ...
    def set_param_value(self, name, value):
        ...
        if (name == "nmodes"):
            oldn = self.parameters["nmodes"].value
        message, success = super(BaseTheoryUCMLAOS, self).set_param_value(
            name, value)
        if not success:
            return message, success
        if (name == "nmodes"):
...
class CLTheoryUCMLAOS(BaseTheoryUCMLAOS, Theory):

    def __init__(self, name="", parent_dataset=None, ax=None):
        super().__init__(name, parent_dataset, ax)

class GUITheoryUCMLAOS(BaseTheoryUCMLAOS, QTheory):
